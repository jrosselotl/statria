...
from app.schemas.test_result_mapping import TestResultCreate, TestResultUpdate, TestResultOut
...
def update_test_result(result_id: int, result: TestResultUpdate, db: Session = Depends(get_db)):
    db_result = db.query(TestResultMapping).filter(TestResultMapping.id == result_id).first()
    if not db_result:
